...
공개 RAG 검색은 사용법·절차를 찾는 기능이다. 고객 장비의 현재 접속자, 실제 주소, 현재 설정 값처럼 해당 환경의 관측이 필요한 직접 질문에는 문서를 근거로 값을 추정하지 않는다. 이런 경우 결과는 비어 있으며 `privacy_mode=summary`의 `evidenceRequirement: live-runtime` 또는 콘솔 안내로 실제 장비 조회 근거가 필요함을 알린다. 기본 배열 응답 형식은 유지한다.

“현재 접속자를 확인하는 방법” 같은 절차 질문과 가정한 장애 시나리오는 문서 검색을 계속 수행한다. 이 구분은 제한된 규칙 기반 분기이며 모든 질문의 의미나 답변 적합성을 판정하는 일반 모델은 아니다. 실제 상태 질문은 문서 수집 부족으로 기록하지 않는다. 이 분기는 장비 조회 권한이나 실행 승인을 부여하지 않으며, 인증된 스코프 검색의 권한 검사는 그대로 유지한다.
