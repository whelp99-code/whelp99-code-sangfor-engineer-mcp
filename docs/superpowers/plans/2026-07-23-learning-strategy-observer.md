...
### CHECKPOINT — 2026-07-28 PR-010 완료

- **Current:** `f34496a` (`feat(learning): add local outbox and Prisma mirror`)
- **REQ 상태:** REQ-22 `PASS`.
- **구현 결과:** canonical local generation과 같은 commit에 pending outbox를 기록하고, stable event ID 기반 Prisma upsert, exponential retry, 10회 실패 DLQ, receipt와 DB-down local success를 구현했다. Prisma schema와 migration은 additive다.
- **검증:** learning store/mirror/schema focused tests, Prisma validate, lint, build 통과.
