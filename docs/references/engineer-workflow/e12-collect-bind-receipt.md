...
This increment adds `bindHciCollectToFieldAcceptanceObservations`. When collect actually executes and a declared target matches the in-process `endpointFor` identity origin, volumes/servers/images facts with `originalPresent === true` are usable by `bindEngineerAuthorizedDeviceReadEvidence`. `collectInventory` now also emits firmware, collectedAt, volume_status_health, and E03B extras as `originalPresent === true` facts when those explicit keys are present on already-fetched REST JSON. Extra provenance is `unofficial_list_key:<id>`, not a device URL. Unofficial extras stay extracted and unverified; they do not become grant-usable surfaces and cannot mint `authorized_device_read` (`REQUIRED_LIVE_SURFACES_NOT_RUN` / `UNOFFICIAL_LIST_KEY_IS_NOT_AUTHORIZED_DEVICE_READ`). Field-qualified lookalikes (`GET /volumes/detail field:firmware`) and bare list reuse (`GET /volumes/detail`) do not bind as authorized-device surfaces. They are not invented from the collect timestamp, a `firmwareVersion` option, volume status, flavor/hypervisor shapes, or provided-only E03B fields. Omitted keys stay absent / `NOT_RUN`. Official HCI OpenAPI/catalog still has no dedicated firmware, hypervisor, network, or HA URL; a real device that only returns volumes/servers/images, and a device whose list JSON happens to contain extra keys, both stay `REQUIRED_LIVE_SURFACES_NOT_RUN`. A named synthetic collector (`e12-collect-bind-synthetic-not-a-device`) can still exercise the binder predicate; that is not a live device read and not `field_accepted`. Mock `:3400`, historical session kinds, fixture inventory without a verified target, and invented collector bytes cannot mint `authorized_device_read`. The fixture workflow path still does not pass `boundObservations` and stays `fieldAccepted: false` / `liveRead: not_run`.
...
| --- | --- | --- |
| `TMPDIR=/home/jm/.cache/sangfor-e12 pnpm exec vitest run --config vitest.config.ts tests/engineer-hci-collection.test.ts tests/engineer-collect-bind.test.ts tests/engineer-field-acceptance.test.ts tests/engineer-workflow-e2e.test.ts tests/engineer-required-observations.test.ts` | 0 | pass (58) |
