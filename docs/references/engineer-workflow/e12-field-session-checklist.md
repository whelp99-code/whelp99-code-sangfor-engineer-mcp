...
When collect actually executes against an authorized device (declared target matches the in-process `endpointFor` identity origin; not mock `:3400`), those REST facts go through `bindHciCollectToFieldAcceptanceObservations` and are usable by `bindEngineerAuthorizedDeviceReadEvidence`. Firmware, collectedAt, volume_status_health, and E03B surfaces pass through only when the already-fetched REST JSON contains those explicit keys and collect returns them as `originalPresent === true` facts that bind through `bindObservedFactToCase` with `unofficial_list_key:<id>` provenance. Field-qualified lookalikes (`GET /volumes/detail field:firmware`) are not bindable device URLs or official catalog paths. Official HCI catalog/OpenAPI still has no dedicated firmware/E03B URL. A volumes/servers/images-only response stays `NOT_RUN` for those extras. Do not invent them from the collect timestamp, a firmware option, volume status, or provided-only E03B fields. Host/CPU/RAM/storage/network/HA stay unsupported unless explicitly provided or actually observed on the wire. Fixture clients, mock console, historical kinds, and invented collector bytes cannot mint `authorized_device_read`. The fixture workflow path still does not pass `boundObservations`.
