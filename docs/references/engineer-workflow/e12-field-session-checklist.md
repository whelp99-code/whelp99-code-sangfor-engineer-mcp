...
When collect actually executes against an authorized device (declared target matches the in-process `endpointFor` identity origin; not mock `:3400`), those REST facts go through `bindHciCollectToFieldAcceptanceObservations` and are usable by `bindEngineerAuthorizedDeviceReadEvidence`. Firmware, collectedAt, volume_status_health, and E03B keys may still be extracted from already-fetched REST JSON as `unofficial_list_key:<id>` extras. Those extras stay unofficial / unverified and cannot mint `authorized_device_read`. Field-qualified lookalikes (`GET /volumes/detail field:firmware`) are not bindable device URLs or official catalog paths. Official HCI catalog/OpenAPI still has no dedicated firmware/E03B URL. A volumes/servers/images-only response, and a list JSON that happens to contain extra keys, both stay `NOT_RUN` for those extras. Do not invent them from the collect timestamp, a firmware option, volume status, or provided-only E03B fields. Host/CPU/RAM/storage/network/HA stay unsupported unless explicitly provided or actually observed on an official catalog URL. Fixture clients, mock console, historical kinds, unofficial list keys, and invented collector bytes cannot mint `authorized_device_read`. The fixture workflow path still does not pass `boundObservations`.
