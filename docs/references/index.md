...
| `playwright` | maintained MCP path: `@sangfor/jm-execution`; legacy `@sangfor/chrome`; standalone login/crawl scripts | `connectOverCDP`, locator strictness (0/>1 match → throw), screenshot capture behind `BrowserExecutionPort`. Operator/verifier/screenshot policy packages do not import it. |
