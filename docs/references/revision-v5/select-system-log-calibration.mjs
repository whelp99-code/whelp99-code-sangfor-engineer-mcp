...
const policyPath = 'docs/references/revision-v5/score-floor-selection-policy-v3.json';
const policyBytes = readFileSync(policyPath), policy = JSON.parse(policyBytes);
if (policy.normalizationSha256 !== hash(readFileSync('packages/sangfor-rag/src/query-normalization.ts'))) throw new Error('Normalization differs from selection policy');
if (policy.normalizationImpactSha256 !== hash(readFileSync('docs/references/revision-v5/system-log-normalization-impact.json'))) throw new Error('Changed input proof');
...
const paths = {
  ...
  'revision-v5-client-korean-development': 'docs/references/revision-v5/full-calibration-client-korean-system-log.json',
