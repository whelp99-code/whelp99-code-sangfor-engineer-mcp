...
- **핵심 게이트 `assertRealExecutionAllowed()`:** 비-dryRun인데 ① `SANGFOR_ALLOW_REAL_EXECUTION !== 'true'` ② production 모드인데 `SANGFOR_ALLOW_PRODUCTION_EXECUTION !== 'true'` ③ 서명 승인 검증 실패(`SANGFOR_OPERATOR_APPROVAL_SECRET`로 HMAC 검증하는 action-bound·만료 서명; 필드 누락/만료/서명 불일치) ④ nonce 재사용(단일사용 store) 중 하나라도 해당하면 throw.
