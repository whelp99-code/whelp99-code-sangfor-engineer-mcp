...
## 4. MCP 서버 (`apps/mcp-server/src/index.ts`, 432줄)
...
- **보안 핵심: `SAFE_TOOL_WHITELIST`** — 읽기 전용 6개 도구(`products`, `search_manuals`, `get_manual_section`, `rag_search`, `rag_index_summary`, `store_health`)만 허용. `WHELP99_ENFORCE_SAFE_TOOLS !== "false"`이면 그 외 도구는 403.
