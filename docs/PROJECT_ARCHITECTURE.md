...
> 갱신일: 2026-08-11
> 기준: 현재 JM modular-monolith / future BLRO working tree
...
2. **다중 환경변수 게이트** — 실제 실행은 `SANGFOR_ALLOW_REAL_EXECUTION`, production mode 또는 모든 non-loopback mutation target은 `SANGFOR_ALLOW_PRODUCTION_EXECUTION` 필요.
...
| 테스트 | Vitest (`tests/*.test.ts`, 152개 스위트) |
...
packages/
  shared/              공통 타입/유틸 (L0 기반)
  sangfor-browser-contracts/ JSON 직렬화 BrowserExecutionPort 계약 (L0)
  sangfor-planner/     프로젝트 분석 + 설정 계획 생성
  sangfor-approval/    텍스트/액션 위험도 분류 + 승인 판정
  sangfor-verifier/    계획 검증 + BrowserExecutionPort 기반 라이브 검증
  sangfor-evidence/    근거 리포트(Markdown/JSON) 생성
  sangfor-evals/       플래너 회귀 평가(품질 게이트)
  sangfor-feedback/    피드백 수집 + 레슨 추출
  sangfor-operator/    operator 세션 + 승인/정책 + port 기반 live 콘솔 조작
  sangfor-jm-execution/ JM 로컬 Playwright/CDP 런타임 + artifact materializer
  sangfor-chrome/      레거시 호환 helper (maintained MCP 브라우저 경로 아님)
...
```
                    ┌──────────────────────────────────────┐
  Cursor/Claude ───▶│  apps/mcp-server (stdio JSON-RPC)     │  ← 108개 sangfor.* 도구
  ...
  evals/feedback             screenshot / operator          collector / finetune
                                     ...
                                     ▼
                            JM local jm-execution ──▶ Playwright / loopback CDP

  AIOSv2 Portal ──REST──▶ apps/http-bridge (3600) ──spawn──▶ mcp-server(stdio)
                          (annotation + signed approval gate)
...
- `tools/list` → 등록된 도구 108개 메타 반환(readOnly/destructive annotations 포함)
...
### 4.1 등록된 MCP 도구 (카테고리별 발췌 — 정본 목록은 코드 레지스트리 `apps/mcp-server/src/index.ts`의 108개)
...
- `verifyResultLive(input)`: 비동기 실장비 검증. mode
  `dry`(기본)/`observe`/`apply`. Verifier는 주입된
  `BrowserExecutionPort`로 read-only observe/verify 요청을 보내며,
  Playwright/CDP/로그인/스크린샷은 JM runtime이 소유한다. 현재
  `apply` mode도 `verify_console`만 수행하며 장비 mutation이나 approval
  소비를 하지 않는다; `SANGFOR_ALLOW_REAL_EXECUTION=true`는 추가
  fail-closed opt-in이다.
...
- **live 경로:** `readLiveConsoleState`와 `executeLiveConsoleAction`은 순수 JSON `BrowserExecutionPort`만 호출한다. Playwright/CDP/page 객체나 자격증명은 operator 경계를 통과하지 않는다.
- **핵심 게이트:** complete-action HMAC(`type/target/value/dryRun/menuPath/formFields`), origin/request 검증, authoritative read-only preflight 뒤 single-use nonce를 mutation 직전에 소비한다.

### 6.9 sangfor-jm-execution — JM 로컬 브라우저 런타임
- `createLocalJmExecutionPort()`가 strict request/result schema, session-origin lock, mutationAttempted/INDETERMINATE 매핑을 담당한다.
- Playwright 드라이버만 browser/page/CDP를 소유하며 loopback 정수 CDP 포트, 단일 intended page, post-action origin lock, opaque `artifact://jm/...` 참조를 강제한다.
- `sangfor-chrome`은 레거시 호환 helper이며 maintained MCP/operator/verifier/evidence 경로에서 직접 import하지 않는다.

### 6.10 sangfor-screenshot — 스크린샷 수집
- `PRODUCT_CONFIGS`: EPP/IAG/CC별 기본 URL과 read-only 메뉴 경로.
- `captureProductScreenshots(options)`: dryRun이면 계획만 반환하고, 실제 캡처는 주입된 `BrowserExecutionPort`와 JM artifact materializer를 사용한다.
...
Prisma/Postgres bridge는 선택적이다. 미설정 시에도 안전·감사·피드백 핵심 경로는 repository file-backed stores를 사용하며, DB 부재가 mutation gate를 우회하지 않는다.
...
| 운영/원격 실행 | `SANGFOR_ALLOW_PRODUCTION_EXECUTION=true` | production 모드 또는 non-loopback target 비-dryRun 추가 요구 |
| 승인 서명 키 | `SANGFOR_OPERATOR_APPROVAL_SECRET` | approval.approvalToken을 action-bound HMAC로 검증(만료·nonce 단일사용 포함) |
...
| HTTP 원격 write gate | `SANGFOR_ALLOW_REMOTE_WRITE` | non-loopback write를 명시적으로 허용; annotations + signed approval은 별도 필수 |
...
**테스트(Vitest, 152개 스위트):** browser/security/operator/verifier/screenshot/observer 경계와 collector, planner, store, adapters, RAG, DOCX/PPTX를 포함한다. 현재 C6 기준 151개 파일 pass, 1개 skip이다.
...
3. **원격 BLRO 승인 미분리:** 현재 complete-action HMAC·만료·nonce 검증은 in-process authority이며, future BLRO network authority로 아직 추출되지 않았다.
4. **선택적 DB bridge:** 핵심 안전·감사 경로는 file-backed이나, 중앙 BLRO DB/ACL/provenance authority는 아직 future extraction 범위다.
...
| 기본 CDP 포트 | 9333(JM managed attach), 9222(console-evidence attach) | jm-execution/screenshot |
| RAG 청크 | 최대 1400자, 180자 오버랩 | rag |
| Hash 임베딩 차원 | 384 | rag |
| LiteLLM/RapidMLX 차원 | 1536 / 768 | rag |
| 리랭크 후보 풀 | 40 → 상위 8 | rag |
| 승인 페이로드 | 필수 메타데이터 + complete canonical action | operator/adapters |
| 감사 로그 보존 | 최소 1년 | knowledge/docx |
| 위험도 순서 | low<medium<high<critical | shared/approval |

---

*본 문서는 2026-08-11 JM modular-monolith / future BLRO 기준으로 갱신되었다. 코드 변경 시 도구 수, 승인 계약, browser ownership, persistence 설명을 함께 갱신할 것.*
