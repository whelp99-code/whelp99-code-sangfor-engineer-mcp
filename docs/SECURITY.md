...
- Engineer field-acceptance grants use `SANGFOR_ENGINEER_FIELD_ACCEPTANCE_SECRET` from the process environment (or the repo's existing secret-loader) only — never a caller-supplied `secret` argument — with the existing domain HMAC primitive. The MAC binds the observation digest of facts re-run through `bindObservedFactToCase` (`originalPresent === true`) plus case/guide revision, not caller-chosen live labels. A valid grant token is consumed through the existing single-use nonce store; replay and a missing/corrupt store fail closed. Missing secret fails closed. The grant is not a device write, does not reuse the operator execution secret, and never enables `SANGFOR_ALLOW_REAL_EXECUTION`.
...
`SANGFOR_ALLOW_REAL_EXECUTION`, `SANGFOR_ALLOW_PRODUCTION_EXECUTION`, `SANGFOR_OPERATOR_APPROVAL_SECRET`, `SANGFOR_IAG_BOOTSTRAP_APPROVAL_SECRET`, `SANGFOR_ALLOW_REMOTE_WRITE`, `SANGFOR_API_TOKEN`, `SANGFOR_BLRO_AUTHORITY_STORE`, `SANGFOR_NONCE_STORE`, `SANGFOR_NONCE_STORE_PATH`, `SANGFOR_PROJECT_ID`, `DATABASE_URL`, `SANGFOR_CHANGE_LEDGER_SECRET`, `SANGFOR_PM_CHAIN_SECRET`, `SANGFOR_WIKI_APPROVAL_SECRET`, `SANGFOR_ENGINEER_FIELD_ACCEPTANCE_SECRET`, `SANGFOR_CAPABILITY_PROMOTION_SECRET`, `SANGFOR_CAPABILITY_PROMOTION_LEDGER_SECRET`, `SANGFOR_CAPABILITY_PROMOTION_CHECKPOINT_SECRET`, `SANGFOR_CAPABILITY_PROMOTION_NONCE_STORE_PATH`, `SANGFOR_CAPABILITY_PROMOTION_LEDGER_PATH`, `SANGFOR_ALLOW_CLOUD_RAG`, `SANGFOR_ALLOW_CLOUD_RAG_CUSTOMER`, `BLRO_BACKUP_DATABASE_URL`, `BLRO_SCRATCH_ADMIN_DATABASE_URL`, `SANGFOR_BLRO_AUDIT_SECRET`. See `.env.example` for the full set.
