...
- Engineer field-acceptance grants use `SANGFOR_ENGINEER_FIELD_ACCEPTANCE_SECRET` with the existing domain HMAC primitive, bound to the live-read digest and case/guide revision. Missing secret fails closed. The grant is not a device write, does not reuse the operator execution secret, and never enables `SANGFOR_ALLOW_REAL_EXECUTION`.
