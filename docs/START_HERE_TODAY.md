...
command. Verified live: **119 tools**.

Sanity check without a client:

```bash
pnpm run smoke:mcp     # observed: smoke-mcp-tools: ok (119 tools)
```
...
`data/evidence` 3.4 MB) and touch every store and all 119 tools. A half-migrated data layer on the
