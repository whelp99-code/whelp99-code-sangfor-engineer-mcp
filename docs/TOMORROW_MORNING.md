...
```bash
pnpm run smoke:mcp     # 확인된 결과: smoke-mcp-tools: ok (119 tools)
```
...
| 감사체인·레지스트리·실행이력·증적·RAG의 Postgres 이전 | 미착수 | 실데이터가 들어있고 119개 도구 전부에 닿습니다. 반쯤 옮긴 상태가 제일 위험합니다 |
...
pnpm run smoke:mcp                 # smoke-mcp-tools: ok (119 tools)
```
