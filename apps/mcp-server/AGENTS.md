...
> MCP stdio JSON-RPC server exposing the whole Sangfor "senior engineer" toolset (108 `sangfor_*` tools). No HTTP port.
...
- Browser-backed tools receive a pure `BrowserExecutionPort` through
  `src/jm-browser-runtime.ts`. Keep Playwright/CDP behavior in
  `@sangfor/jm-execution`; policy handlers in `src/index.ts` must not drive a
  page, import Playwright, or expose local credentials/artifact paths.

## Working here
- Widest fan-in in the repo. Import by relative path (`../../../packages/<pkg>/src/index.js`).
