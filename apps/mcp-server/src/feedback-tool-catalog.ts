...
import { submitFeedbackWithAuthority, extractLessonWithAuthority } from '../../../packages/sangfor-feedback/src/index.js';
...
export const feedbackToolCatalog: readonly ToolCatalogEntry[] = [
  ...
  ["sangfor_submit_feedback", {
    ...
    handler: async (args: Parameters<typeof submitFeedbackWithAuthority>[0]) => {
      const event = await submitFeedbackWithAuthority(args, mcpLocalAuthority('feedback_lessons', feedbackRoot()));
  ...
  ["sangfor_extract_lesson", {
    ...
    handler: ({ feedbackId }) => extractLessonWithAuthority(feedbackId, mcpLocalAuthority('feedback_lessons', feedbackRoot()))
  }],
  ["sangfor_propose_wiki_update", {
    ...
    handler: (input: Parameters<typeof proposeWikiUpdateWithAuthority>[0]) => proposeWikiUpdateWithAuthority(input, mcpLocalAuthority('wiki_proposals', wikiRoot()))
  }],
  ["sangfor_approve_wiki_update", {
    ...
    handler: ({ proposalId, decision, token, reviewer }) => approveWikiUpdateWithAuthority(proposalId, decision, { token, reviewer }, mcpLocalAuthority('wiki_proposals', wikiRoot()))
  }],
  ["sangfor_apply_wiki_update", {
    ...
    handler: ({ proposalId }) => applyWikiUpdateWithAuthority(proposalId, mcpLocalAuthority('wiki_proposals', wikiRoot()))
  }],
  ["sangfor_apply_obsidian_wiki_update", {
    ...
    handler: ({ proposalId, vaultPath }) => applyObsidianWikiUpdateWithAuthority({
  ...
  ["sangfor_apply_github_wiki_update", {
    ...
    handler: ({ proposalId, repoUrl, localPath }) => {
      ...
      return applyGitHubWikiUpdateWithAuthority({
