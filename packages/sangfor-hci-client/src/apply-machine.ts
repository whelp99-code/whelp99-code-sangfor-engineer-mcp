...
export async function applyCreateVolume(
  ...
  const step = async (state: ApplyState, detail: string): Promise<void> => {
    const ev: ApplyEvent = { at: new Date().toISOString(), state, detail };
    events.push(ev);
    await ledger.append(runId, 'state', ev);
  };
  const halt = async (detail: string, extra: Partial<ApplyResult> = {}): Promise<ApplyResult> => {
    await step('FAILED_HALT', `${detail} — halting for human review (no auto-rollback)`);
    return { ok: false, finalState: 'FAILED_HALT', runId, events, ...extra };
  };

  await step('PENDING', `create-volume '${input.name}' (${input.sizeGb}GB)`);

  await step('VALIDATING', 'input validation');
  const problems = validateCreateVolumeInput(input);
  if (problems.length) return halt(`validation failed: ${problems.join('; ')}`);

  await step('APPLYING', 'POST /volumes with X-Client-Token idempotency');
  await ledger.append(runId, 'request', { op: 'create-volume', name: input.name, sizeGb: input.sizeGb, description: input.description ?? null, clientToken: input.clientToken });
  ...
  await ledger.append(runId, 'response', { status: created.status, volume: created.volume });
  ...
  await step('VERIFYING', 'independent GET read-back');
  const readBack = await readBackVolume(client, { volumeId: created.volume?.id, name: input.name, sizeGb: input.sizeGb }, opts);
  await ledger.append(runId, 'verdict', readBack);

  if (readBack.verdict === 'PASS') {
    await step('SUCCEEDED', `volume ${readBack.volumeId} verified by read-back`);
