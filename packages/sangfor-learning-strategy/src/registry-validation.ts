...
const REGISTRY_ENTRY_KEYS = [
  'adapterProduct', 'vendor', 'aliases', 'observerOnlyAliases', 'observerEligible', 'defaultSpecMapping', 'specMappingByVariant',
] as const;
...
function normalizedEntry(entry: unknown): ProductRegistryEntry {
  ...
  if (!normalizedProduct) invalidRegistry('Registry identity has invalid required fields.');
...
export function validateProductRegistryView(view: ProductRegistryView): ProductRegistryView {
  ...
  return deepFreeze({ schemaVersion: 1, registryDigest, entries: normalizedEntries });
}
