...
export function classifyTextRisk(text: string): RiskLevel {
  ...
  if (['delete', 'shutdown', 'factory reset', 'drop', 'format'].some(term => value.includes(term))) return 'critical';
  if (DANGEROUS_TERMS.some(term => value.includes(term))) return 'high';
  if (['network', 'policy', 'storage', 'migration', 'agent deployment'].some(term => value.includes(term))) return 'medium';
  return 'low';
}
