...
describe('Registry device mutators — lock-protected read-modify-write', () => {
  ...
  it('createDevice actually acquires devices.json.lock: a pre-held lock blocks it, then it succeeds once released', () => {
    ...
    expect(() => reg.createDevice({ name: 'dev1', product: 'HCI_SCP', host: '10.0.0.1' })).toThrow(/LOCK_TIMEOUT/);
    expect(reg.devices()).toEqual([]); // blocked attempt left no partial write

    rmdirSync(lockPath);
    const device = reg.createDevice({ name: 'dev1', product: 'HCI_SCP', host: '10.0.0.1' });
    expect(device.name).toBe('dev1');
    expect(existsSync(lockPath)).toBe(false); // released after success
  }, 10_000);

  it('20 sequential createDevice calls each land under their own lock/release cycle with no lost or duplicated writes', () => {
    const reg = new Registry(dir);
    for (let i = 0; i < 20; i += 1) {
      reg.createDevice({ name: `dev-${i}`, product: 'HCI_SCP', host: `10.0.0.${i}` });
    }
...
describe('PlaybookStore — lock-protected read-modify-write', () => {
  ...
  it('create actually acquires playbooks.json.lock: a pre-held lock blocks it, then it succeeds once released', () => {
