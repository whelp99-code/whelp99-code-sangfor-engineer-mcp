...
const report = {
  ...
  missingRelevantSources: [], forbiddenHits: 0, meanLatencyMs: 100, p95LatencyMs: 120,
};
describe('offline corpus quality gate', () => {
  ...
  it.each([
    ...
    { ...report, missingRelevantSources: ['missing.md'] },
  ...
  it('refuses missing baseline answers even when candidate metrics pass', () => {
    expect(compareCorpusQuality(report, { ...report, missingRelevantSources: ['missing.md'] }, thresholds))
      .toMatchObject({ decision: 'NOT_ELIGIBLE', checks: { relevantSourcesPresent: false } });
  });
  it('requires an explicit answer-source availability measurement', () => {
    const { missingRelevantSources, ...unmeasured } = report;
    expect(() => compareCorpusQuality(unmeasured, report, thresholds)).toThrow();
  });
