...
describe('@sangfor/chronicle — unapproved-drift read model', () => {
  ...
  async function seedChain(): Promise<{ first: string; second: string; third: string }> {
    const first = (await recordSnapshot({ ...base, observed: { firmware: '8.0.75' }, capturedAt: '2026-08-01T00:00:00.000Z', dir , authority: testLocalWriteAuthority('config_chronicle_state', dir)})).hash;
    const second = (await recordSnapshot({ ...base, observed: { firmware: '8.0.80' }, capturedAt: '2026-08-02T12:00:00.000Z', dir , authority: testLocalWriteAuthority('config_chronicle_state', dir)})).hash;
    const third = (await recordSnapshot({ ...base, observed: { firmware: '8.0.80', ntpServer: '10.0.0.1' }, capturedAt: '2026-08-09T09:30:00.000Z', dir , authority: testLocalWriteAuthority('config_chronicle_state', dir)})).hash;
    return { first, second, third };
  }

  it('reports a finding for every drift when there are no approvals at all', async () => {
    const { second, third } = await seedChain();
  ...
  it('suppresses a finding whose capturedAt falls inside an approval window for that device', async () => {
    const { third } = await seedChain();
  ...
  it('ignores approvals belonging to a different device', async () => {
    const { second, third } = await seedChain();
    const approvals: readonly ChangeApproval[] = [
      {
  ...
  it('treats an approval with no explicit window as open-ended from approvedAt', async () => {
    const { second, third } = await seedChain();
    const approvals: readonly ChangeApproval[] = [
  ...
  it('never reports the genesis snapshot as drift and returns [] for an unknown device', async () => {
    await recordSnapshot({ ...base, observed: { firmware: '8.0.75' }, capturedAt: '2026-08-01T00:00:00.000Z', dir , authority: testLocalWriteAuthority('config_chronicle_state', dir)});
  ...
  it('is a pure read model: it writes nothing (no new files, no mtime change)', async () => {
    await seedChain();
  ...
  it('reports every changed key of a multi-key drift, sorted', async () => {
    await recordSnapshot({ ...base, observed: { a: 1, b: 2, c: 3 }, capturedAt: '2026-08-01T00:00:00.000Z', dir , authority: testLocalWriteAuthority('config_chronicle_state', dir)});
    const h = (await recordSnapshot({ ...base, observed: { a: 9, c: 3, d: 4 }, capturedAt: '2026-08-02T00:00:00.000Z', dir , authority: testLocalWriteAuthority('config_chronicle_state', dir)})).hash;
