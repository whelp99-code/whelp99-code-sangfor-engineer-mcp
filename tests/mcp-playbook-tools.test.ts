...
const READ_TOOLS = [
  'sangfor_playbook_list', 'sangfor_playbook_get',
  'sangfor_playbook_run_status', 'sangfor_playbook_agent_tasks',
];
const WRITE_TOOLS = [
...
describe('MCP 플레이북 도구 — 타워 프록시', () => {
  it('읽기 도구가 타워 GET 경로와 토큰 헤더를 그대로 사용한다', async () => {
    seen.length = 0;
    await getToolHandler('sangfor_playbook_list')!({});
    await getToolHandler('sangfor_playbook_get')!({ playbookId: 'pb_1' });
    await getToolHandler('sangfor_playbook_run_status')!({ playbookRunId: 'pbrun_1' });
    await getToolHandler('sangfor_playbook_agent_tasks')!({});
  ...
  it('쓰기 도구가 타워 POST/PATCH 바디를 정확히 구성한다', async () => {
    seen.length = 0;
    const blocks = [{ id: 'b1', type: 'tool', toolId: 'sangfor_store_health', args: {} }, { id: 'r', type: 'report' }];
    await getToolHandler('sangfor_playbook_create')!({ name: 'n', goal: 'g', authoredBy: 'agent:claude', blocks });
    await getToolHandler('sangfor_playbook_add_revision')!({ playbookId: 'pb_1', authoredBy: 'agent:claude', blocks });
    await getToolHandler('sangfor_playbook_execute')!({ playbookId: 'pb_1' });
    await getToolHandler('sangfor_playbook_submit_analysis')!({ playbookRunId: 'pbrun_1', playbookId: 'pb_1', summary: 's', authoredBy: 'agent:claude' });
    await getToolHandler('sangfor_playbook_close_agent_task')!({ taskId: 'atask_1', result: { playbookId: 'pb_1' } });
    expect(seen.map((s) => `${s.method} ${s.url}`)).toEqual([
      'POST /api/playbooks',
  ...
  it('타워 4xx는 예외가 아니라 error 값으로 돌아온다', async () => {
    const out = await getToolHandler('sangfor_playbook_get')!({ playbookId: 'pb_missing' }) as { error?: string };
  ...
  it('타워가 안 떠 있으면 실행 힌트를 담은 error 값을 반환한다', async () => {
    ...
    try {
      const out = await getToolHandler('sangfor_playbook_list')!({}) as { error?: string; hint?: string };
