...
describe('@sangfor/chronicle — content-addressed snapshot store', () => {
  ...
  it('canonicalizes recursively-sorted keys and excludes ephemeral keys from the hash', () => {
  ...
  it('records a first snapshot with a sha256 content address, no parent, and an all-added diff', () => {
  ...
  it('links the next snapshot to the previous head and computes a semantic write-time diff', () => {
  ...
  it('creates no new node for an identical re-record and returns the existing head', () => {
    const observed = { firmware: '8.0.75', haEnabled: true };
    const first = recordSnapshot({ deviceId: 'dev-1', observed, capturedAt: '2026-08-01T00:00:00.000Z', dir });
    const again = recordSnapshot({ deviceId: 'dev-1', observed, capturedAt: '2026-08-05T00:00:00.000Z', dir });
  ...
  it('creates no new node when only ephemeral keys differ, but they are still stored on the head', () => {
    const first = recordSnapshot({
      deviceId: 'dev-1',
  ...
  it('keeps one independent chain per deviceId', () => {
    const a = recordSnapshot({ deviceId: 'dev-a', observed: { x: 1 }, capturedAt: '2026-08-01T00:00:00.000Z', dir });
    const b = recordSnapshot({ deviceId: 'dev-b', observed: { x: 2 }, capturedAt: '2026-08-01T00:00:00.000Z', dir });
    recordSnapshot({ deviceId: 'dev-a', observed: { x: 3 }, capturedAt: '2026-08-02T00:00:00.000Z', dir });
  ...
  it('listSnapshots returns the chain oldest-first with intact parent links', () => {
    const h1 = recordSnapshot({ deviceId: 'dev-1', observed: { x: 1 }, capturedAt: '2026-08-01T00:00:00.000Z', dir }).hash;
    const h2 = recordSnapshot({ deviceId: 'dev-1', observed: { x: 2 }, capturedAt: '2026-08-02T00:00:00.000Z', dir }).hash;
    const h3 = recordSnapshot({ deviceId: 'dev-1', observed: { x: 3 }, capturedAt: '2026-08-03T00:00:00.000Z', dir }).hash;
  ...
  it('getDiff returns the stored write-time diff for the head, or between two hashes', () => {
    const h1 = recordSnapshot({ deviceId: 'dev-1', observed: { x: 1, y: 'a' }, capturedAt: '2026-08-01T00:00:00.000Z', dir }).hash;
    const h2 = recordSnapshot({ deviceId: 'dev-1', observed: { x: 2, y: 'a' }, capturedAt: '2026-08-02T00:00:00.000Z', dir }).hash;
    const h3 = recordSnapshot({ deviceId: 'dev-1', observed: { x: 2, y: 'b' }, capturedAt: '2026-08-03T00:00:00.000Z', dir }).hash;
  ...
  it('persists JSON files under the given dir and never touches the repo', () => {
    recordSnapshot({ deviceId: 'dev-1', observed: { x: 1 }, capturedAt: '2026-08-01T00:00:00.000Z', dir });
  ...
  it('holds a directory lock across the read-modify-write (a pre-held lock blocks the write)', () => {
    recordSnapshot({ deviceId: 'dev-1', observed: { x: 1 }, capturedAt: '2026-08-01T00:00:00.000Z', dir });
    ...
    expect(() =>
      recordSnapshot({ deviceId: 'dev-1', observed: { x: 2 }, capturedAt: '2026-08-02T00:00:00.000Z', dir }),
  ...
  it('rejects a deviceId that would escape the store directory', () => {
    expect(() =>
      recordSnapshot({ deviceId: '../escape', observed: { x: 1 }, capturedAt: '2026-08-01T00:00:00.000Z', dir }),
    ).toThrow(/deviceId/);
    expect(readdirSync(dir)).toEqual([]);
  });

  it('stores canonical JSON on disk so the file content itself is the hashed preimage', () => {
    const r = recordSnapshot({
      deviceId: 'dev-1',
      observed: { b: 2, a: 1 },
      capturedAt: '2026-08-01T00:00:00.000Z',
