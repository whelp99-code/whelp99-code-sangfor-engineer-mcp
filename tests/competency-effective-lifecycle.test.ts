...
const setup = (campaign: 'api_read_only' | 'browser' = 'api_read_only') => {
  const value = createEffectiveFixture(campaign);
  fixtures.push(value);
  value.ledger.append(value.event({ index: 1, action: 'promote', fromMaturity: 'tested_mock', toMaturity: 'field_verified' }));
  return value;
};
