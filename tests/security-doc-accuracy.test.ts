...
describe('docs/SECURITY.md — 브리지 게이트 서술이 실제 동작과 일치한다', () => {
  it('승인 없는 destructive는 모든 토글과 무관하게 거부된다', () => {
    const d = authorizeToolCall({
  ...
  it('유효한 bridge.tool-call 승인이 붙은 destructive는 그 1회에 한해 허용된다', () => {
  ...
  it('문서가 "always refused"로 단정하지 않고 승인 경로를 함께 서술한다', () => {
