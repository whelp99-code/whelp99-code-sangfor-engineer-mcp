...
type WorkItem = ReturnType<typeof generateExcelBasedChangePlan>['workPlan'][number];
...
function grouped(items: WorkItem[]): Record<string, WorkItem[]> {
...
function buildDocumentXml(): string {
  const plan = generateExcelBasedChangePlan({ filePath: EXCEL_PATH, prioritizeOnly: true });
...
function main(): void {
  ...
  mkdirSync(join(WORK_DIR, '_rels'), { recursive: true });
  mkdirSync(join(WORK_DIR, 'word/_rels'), { recursive: true });
  writeFileSync(join(WORK_DIR, '[Content_Types].xml'), contentTypesXml);
  writeFileSync(join(WORK_DIR, '_rels/.rels'), relsXml);
  writeFileSync(join(WORK_DIR, 'word/document.xml'), buildDocumentXml());
  writeFileSync(join(WORK_DIR, 'word/styles.xml'), stylesXml);
  writeFileSync(join(WORK_DIR, 'word/numbering.xml'), numberingXml);
  writeFileSync(join(WORK_DIR, 'word/_rels/document.xml.rels'), documentRelsXml);
  if (existsSync(DOCX_PATH)) rmSync(DOCX_PATH);
  execFileSync('zip', ['-qr', DOCX_PATH, '.'], { cwd: WORK_DIR });
