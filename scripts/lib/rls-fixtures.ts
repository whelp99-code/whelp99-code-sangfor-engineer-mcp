...
export async function seedRlsProject(database: SqlExecutor, fixture: RlsProjectFixture): Promise<void> {
  ...
  const statements: readonly [string, readonly unknown[]][] = [
    ...
    [`INSERT INTO "BlroRagIndexPromotion" ("tenantId","projectId","cohortId","indexEpoch","report","reportCanonical","reportDigest","state","reason","promotedAt") VALUES ($1,$2,$3,33,'{}','{}',$4,'promoted','rls probe',now())`, [tenantId, projectId, `cohort-${label}`, digest('promotion')]],
    [`INSERT INTO "BlroRagIndexPromotionEvidence" ("tenantId","projectId","nonce","cohortId","indexEpoch","authorityActorId","evidence","evidenceCanonical","reportDigest") VALUES ($1,$2,$3,$4,33,$5,'{}','{}',$6)`, [tenantId, projectId, `promotion-nonce-${label}`, `cohort-${label}`, actorId, digest('promotion')]],
  ...
  await execute(database, `INSERT INTO "BlroEngineerCase" ("id","tenantId","projectId","actorId","revision","guideRevision","guideDigest","observationDigest","requirementDigest","requestId","requestDigest","environmentKind","originalPresent","document") VALUES ($1,$2,$3,$4,'rev-probe','guide-probe',$5,$6,$7,$8,$9,'fixture',false,'{}')`, [`case-${label}`, tenantId, projectId, actorId, digest('case-guide'), digest('case-obs'), digest('case-req'), `request-${label}`, digest('case-request')]);
  await execute(database, `INSERT INTO "BlroEngineerCaseArtifact" ("id","tenantId","projectId","actorId","caseId","digest","mediaType","payload","sanitized","retention") VALUES ($1,$2,$3,$4,$5,$6,'application/json','{}',true,'case-revision')`, [`case-artifact-${label}`, tenantId, projectId, actorId, `case-${label}`, digest('case-artifact')]);
}
