...
const outputPath = process.argv[3] ?? 'data/evals/issue-16/ab-evaluation.json';
const baselineIndex = process.argv[4] ?? 'data/evals/issue-16/baseline-reconciled-index.json';
const candidateIndex = process.argv[5] ?? 'data/evals/issue-16/e5-reconciled-index.json';
const benchmark = JSON.parse(readFileSync(qrelsPath, 'utf8'));
const { queries, noAnswerQueries, k, thresholds } = benchmark;

const hciQueryIds = new Set(['hci-cli-6113', 'hci-overlay-6111', 'hci-datastore', 'hci-iscsi-ko', 'backup-space-case', 'hci-system-diagnostics', 'hci-raid-ko', 'hci-api-reference', 'hci-dfw-ko', 'vcenter-cert-case']);
const searchVersions = {
  'hci-cli-6113': '6.11.3',
  'ndr-3098-preupgrade': '3.0.98',
  'ndr-resolved-issues': '3.0.98',
  'ngfw-80107-guaranteed': '8.0.107',
  'ngfw-80106-limited': '8.0.106',
  'hci-overlay-6111': '6.11.1R1',
  'scp-612-scheduled-backup': '6.12.0'
};
function searchProduct(query) {
  if (hciQueryIds.has(query.queryId)) return 'HCI';
  if (query.queryId === 'no-ngfw-9999') return 'NGFW';
...
async function runArm({ name, indexPath, baseUrl, model }) {
  ...
  let diagnostics;
  try {
    await rpc('initialize', { protocolVersion: '2024-11-05', capabilities: {}, clientInfo: { name: `issue-16-${name}`, version: '1.0' } });
    for (const query of [...queries, ...noAnswerQueries]) {
      const queryStarted = performance.now();
      const result = await rpc('tools/call', { name: 'sangfor_rag_search', arguments: { query: query.query, product: searchProduct(query), ...(searchVersions[query.queryId] ? { version: searchVersions[query.queryId] } : {}), sourceType: 'manual', trustLevel: 'official', limit: k, indexPath, privacy_mode: 'raw' } });
