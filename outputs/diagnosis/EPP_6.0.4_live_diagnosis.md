...
- 요약: 잘못됨 0 · 추가 필요 0 · 환경 의존 0 · 판정 불가 10 · 정상 4
- 종합 판정(ok): **조치 필요**
...
## 판정 불가 (indeterminate — 설정값 미확인/근거 부족) (10)
...
- **에이전트 자동 업데이트 활성** (기대: true, 실제: 확인 불가) 
...
- **디바이스 컨트롤 정책 구성됨(환경 의존)** (기대: true, 실제: 확인 불가) 
  - 근거: Sangfor Endpoint Secure 6.0.4 User Manual — Defense / Device Control

- **예외(exclusion) 목록 관리됨(환경 의존)** (기대: true, 실제: 확인 불가) 
...
- **취약점/패치 DB가 최신 상태 (Vulnerability patch DB is up to date)** (기대: true, 실제: true) 
  ...
  - 출처: support.sangfor.com EPP 6.0.4 User Manual 
  - 관측(수집기 주장, 미검증): POST /api/edrgoweb/v1/patch/statistics @ 2026-07-07T10:16:21.392Z [live-xhr]

- **미조치 취약점 수가 0 (권장)** (기대: 0, 실제: 0) 
  - 근거: Athena EPP 6.0.4 User Manual — Vulnerabilities / Vuln List
  - 출처: support.sangfor.com EPP 6.0.4 User Manual 
  - 관측(수집기 주장, 미검증): POST /api/edrgoweb/v1/vulner/list/homepageVulner @ 2026-07-07T10:16:21.392Z [live-xhr]
...
- **악성 도메인 탐지 활성 (malicious domain detection active)** (기대: true, 실제: true) 
  ...
  - 출처: support.sangfor.com EPP 6.0.4 User Manual 
  - 관측(수집기 주장, 미검증): POST /api/edrgoweb/v1/domain_detect/get_domain_info @ 2026-07-07T10:16:21.392Z [live-xhr]
...
- **자산 인벤토리 분류 구성됨 (asset inventory classified)** (기대: 1, 실제: 5) 
  ...
  - 출처: support.sangfor.com EPP 6.0.4 User Manual 
  - 관측(수집기 주장, 미검증): POST /api/edrgoweb/v1/asset/inventory/classify @ 2026-07-07T10:16:21.392Z [live-xhr]

## 커버리지 (감사 범위)

- 스펙 항목 14개 중 관측값 미확인 10개 (security_baseline_configured, malware_scan_schedule_enabled, vuln_defs_current, dar_monitoring_active, endpoint_isolation_policy_present, agent_auto_update_enabled, quarantine_policy_present, edr_behavior_monitoring_enabled, device_control_policy_present, exclusion_list_managed)
...
> 수집: 10.80.1.106 라이브 콘솔 XHR 12개 엔드포인트 (read-only)
