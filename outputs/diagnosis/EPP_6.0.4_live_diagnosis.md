...
- 요약: 잘못됨 0 · 추가 필요 1 · 환경 의존 1 · 판정 불가 2 · 정상 10
- 종합 판정(ok): **조치 필요**
...
- **에이전트 자동 업데이트 활성** (기대: true, 실제: false) 
  - 근거: Sangfor Endpoint Secure 6.0.4 User Manual — Update / Agent Update Policy 
  - 관측(수집기 주장, 미검증): POST /launch.php?opr=get_upgrade_state @ 2026-07-07T23:33:47.446Z [live-xhr]
...
- **디바이스 컨트롤 정책 구성됨(환경 의존)** (기대: true, 실제: false) 
  ...
  - 관측(수집기 주장, 미검증): POST /api/edrgoweb/v1/control/queryctrlapppolicy @ 2026-07-07T23:33:47.446Z [live-xhr]

## 판정 불가 (indeterminate — 설정값 미확인/근거 부족) (2)
...
- **엔드포인트 격리 정책 구성됨** (기대: true, 실제: 확인 불가) 
...
- **취약점/패치 DB가 최신 상태 (Vulnerability patch DB is up to date)** (기대: true, 실제: true) 
  ...
  - 출처: support.sangfor.com EPP 6.0.4 User Manual 
  - 관측(수집기 주장, 미검증): POST /api/edrgoweb/v1/patch/statistics @ 2026-07-07T23:33:47.446Z [live-xhr]

- **보안 베이스라인(Compliance Baseline) 규칙이 1개 이상 구성됨** (기대: 1, 실제: 1) 
  - 근거: Athena EPP 6.0.4 User Manual — Compliance Check / Baseline
  - 출처: support.sangfor.com EPP 6.0.4 User Manual 
  - 관측(수집기 주장, 미검증): POST /api/edrgoweb/v1/baseline/getRule @ 2026-07-07T23:33:47.446Z [live-xhr]

- **미조치 취약점 수가 0 (권장)** (기대: 0, 실제: 0) 
  - 근거: Athena EPP 6.0.4 User Manual — Vulnerabilities / Vuln List
  - 출처: support.sangfor.com EPP 6.0.4 User Manual 
  - 관측(수집기 주장, 미검증): POST /api/edrgoweb/v1/vulner/list/homepageVulner @ 2026-07-07T23:33:47.446Z [live-xhr]

- **취약점 정의 DB에 미적용 업데이트 없음** (기대: false, 실제: false) 
  - 근거: Athena EPP 6.0.4 User Manual — Vulnerabilities / Version
  - 출처: support.sangfor.com EPP 6.0.4 User Manual 
  - 관측(수집기 주장, 미검증): POST /api/edrgoweb/v1/vulner/list/version @ 2026-07-07T23:33:47.446Z [live-xhr]

- **Data-at-Risk(민감데이터) 모니터링 활성** (기대: true, 실제: true) 
  - 근거: Athena EPP 6.0.4 User Manual — Data at Risk
  - 출처: support.sangfor.com EPP 6.0.4 User Manual 
  - 관측(수집기 주장, 미검증): POST /api/edrgoweb/v1/cnapp/professional/dar/webapi/interval/status @ 2026-07-07T23:33:47.446Z [live-xhr]

- **격리(quarantine) 정책 구성됨** (기대: true, 실제: true) 
  - 근거: Sangfor Endpoint Secure 6.0.4 User Manual — Response / Quarantine 
  - 관측(수집기 주장, 미검증): POST /launch.php?opr=list_policy_safe_area @ 2026-07-07T23:33:47.446Z [live-xhr]

- **EDR 행위 모니터링 활성** (기대: true, 실제: true) 
  - 근거: Sangfor Endpoint Secure 6.0.4 User Manual — Defense / EDR Behavior Monitoring 
  - 관측(수집기 주장, 미검증): POST /launch.php?opr=list_policy_extortion_protection @ 2026-07-07T23:33:47.446Z [live-xhr]

- **예외(exclusion) 목록 관리됨(환경 의존)** (기대: true, 실제: true) 
  - 근거: Sangfor Endpoint Secure 6.0.4 User Manual — Policy / Exclusions 
  - 관측(수집기 주장, 미검증): POST /launch.php?opr=list_policy_trust_path @ 2026-07-07T23:33:47.446Z [live-xhr]
...
- **악성 도메인 탐지 활성 (malicious domain detection active)** (기대: true, 실제: true) 
  ...
  - 출처: support.sangfor.com EPP 6.0.4 User Manual 
  - 관측(수집기 주장, 미검증): POST /api/edrgoweb/v1/domain_detect/get_domain_info @ 2026-07-07T23:33:47.446Z [live-xhr]
...
- **자산 인벤토리 분류 구성됨 (asset inventory classified)** (기대: 1, 실제: 5) 
  ...
  - 출처: support.sangfor.com EPP 6.0.4 User Manual 
  - 관측(수집기 주장, 미검증): POST /api/edrgoweb/v1/asset/inventory/classify @ 2026-07-07T23:33:47.446Z [live-xhr]

## 커버리지 (감사 범위)

- 스펙 항목 14개 중 관측값 미확인 2개 (malware_scan_schedule_enabled, endpoint_isolation_policy_present)
...
> 수집: 10.80.1.106 라이브 콘솔 XHR 65개 엔드포인트 (read-only)
